...
class Model(object):
    def __init__(self, infer_type: str) -> None:
    ...
    def setup(self) -> None:
        ...
        self.ort_session = ort.InferenceSession(self.path, providers=["AzureExecutionProvider", "CPUExecutionProvider"])
    ...
    def infer(self, image: np.ndarray) -> Union[str, list, tuple, np.ndarray]:
        ...
        if re.match(r"^classify$", self.infer_type, re.IGNORECASE):
            ...
            disp_image = cv2.resize(src=Helpers().segmenter_decode(class_index_image), dsize=(image_w, image_h), interpolation=cv2.INTER_AREA)
